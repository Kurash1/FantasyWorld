...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT In ) : PDX_COLOR
		{
			...
			float rbx[6][4] = { // top left x, top left y, width, height
				{ 2092, 360, 94, 127 },
				{ 1811, 487, 532, 275 },
				{ 2174, 759, 171, 182 },
				{ 2331, 506, 301, 107 },
			...
			float x = In.vPrePos_Fade.x;
			float y = (In.vPrePos_Fade.z - 2816) * -1;

			for (int i = 0; i < 6; i++) {
				float left = rbx[i][0];
				float top = rbx[i][1];
				float right = left + rbx[i][2];
				float bottom = top + rbx[i][3];

				if (x >= left && x <= right && y >= top && y <= bottom) {
					float tBrightness = (vColor.r + vColor.g + vColor.b) / 9;
					vColor = float3(0.3 + tBrightness, tBrightness, tBrightness);
					break;
				}
			}
